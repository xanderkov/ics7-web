...
// QueryIs chains the current query on the "is" edge.
func (aq *AccountQuery) QueryIs() *DoctorQuery {
...
// WithIs tells the query-builder to eager-load the nodes that are connected to
// the "is" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithIs(opts ...func(*DoctorQuery)) *AccountQuery {
	query := (&DoctorClient{config: aq.config}).Query()
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withIs; query != nil {
		if err := aq.loadIs(ctx, query, nodes,
			func(n *Account) { n.Edges.Is = []*Doctor{} },
			func(n *Account, e *Doctor) { n.Edges.Is = append(n.Edges.Is, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AccountQuery) loadIs(ctx context.Context, query *DoctorQuery, nodes []*Account, init func(*Account), assign func(*Account, *Doctor)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(account.IsTable)
		s.Join(joinT).On(s.C(doctor.FieldID), joinT.C(account.IsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(account.IsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(account.IsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "is" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
