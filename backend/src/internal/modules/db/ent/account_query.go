...
func (aq *AccountQuery) loadIs(ctx context.Context, query *DoctorQuery, nodes []*Account, init func(*Account), assign func(*Account, *Doctor)) error {
	...
	query.Where(predicate.Doctor(func(s *sql.Selector) {
		s.Where(sql.InValues(account.IsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.account_is
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_is" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_is" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
