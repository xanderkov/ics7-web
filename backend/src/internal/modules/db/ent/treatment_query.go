...
func (tq *TreatmentQuery) Order(o ...treatment.Order) *TreatmentQuery {
...
func (tq *TreatmentQuery) All(ctx context.Context) ([]*Treatment, error) {
	...
	qr := querierAll[[]*Treatment, *TreatmentQuery]()
	return withInterceptors[[]*Treatment](ctx, tq, qr, tq.inters)
}
...
func (tq *TreatmentQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (tq *TreatmentQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, tq, querierCount[*TreatmentQuery](), tq.inters)
}
...
//		GroupBy(treatment.FieldTablets).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
func (tq *TreatmentQuery) GroupBy(field string, fields ...string) *TreatmentGroupBy {
	tq.ctx.Fields = append([]string{field}, fields...)
	grbuild := &TreatmentGroupBy{build: tq}
	grbuild.flds = &tq.ctx.Fields
	grbuild.label = treatment.Label
...
//
//	client.Treatment.Query().
//		Select(treatment.FieldTablets).
//		Scan(ctx, &v)
func (tq *TreatmentQuery) Select(fields ...string) *TreatmentSelect {
	tq.ctx.Fields = append(tq.ctx.Fields, fields...)
	sbuild := &TreatmentSelect{TreatmentQuery: tq}
	sbuild.label = treatment.Label
	sbuild.flds, sbuild.scan = &tq.ctx.Fields, sbuild.Scan
	return sbuild
}

// Aggregate returns a TreatmentSelect configured with the given aggregations.
func (tq *TreatmentQuery) Aggregate(fns ...AggregateFunc) *TreatmentSelect {
...
func (tq *TreatmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Treatment, error) {
	var (
		nodes       = []*Treatment{}
		_spec       = tq.querySpec()
		loadedTypes = [1]bool{
	...
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*Treatment).scanValues(nil, columns)
	}
...
func (tq *TreatmentQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(treatment.Table, treatment.Columns, sqlgraph.NewFieldSpec(treatment.FieldID, field.TypeInt))
	_spec.From = tq.sql
...
func (tgb *TreatmentGroupBy) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, tgb.build.ctx, "GroupBy")
	if err := tgb.build.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*TreatmentQuery, *TreatmentGroupBy](ctx, tgb.build, tgb, tgb.build.inters, v)
}

func (tgb *TreatmentGroupBy) sqlScan(ctx context.Context, root *TreatmentQuery, v any) error {
...
func (ts *TreatmentSelect) Aggregate(fns ...AggregateFunc) *TreatmentSelect {
	ts.fns = append(ts.fns, fns...)
	return ts
}

// Scan applies the selector query and scans the result into the given value.
func (ts *TreatmentSelect) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, ts.ctx, "Select")
	if err := ts.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*TreatmentQuery, *TreatmentSelect](ctx, ts.TreatmentQuery, ts, ts.inters, v)
}

func (ts *TreatmentSelect) sqlScan(ctx context.Context, root *TreatmentQuery, v any) error {
